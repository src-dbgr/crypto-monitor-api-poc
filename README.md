...
## Purpose of the this application

- This application is part of a POC which aims the at following
  - Connecting multiple Docker based services in order to establish a datasource for Grafana dashboards where monitoring of cryptocurrency information shall take place
  - This maven based Spring Boot backend application shall provide a REST API which handles CRUD operations persisted in a PostgreSQL database for Cryptocurrency information such as price performance over time
  - For price updates a separate crypto client is in place (the client is covered separately) which serves as the man in the middle between the cryptocurrency webservice (in this case coingecko.com) and this Spring Boot backend server
...
> **Note:** The following description contains the steps you need to take start the application and also useful background information which I prefix with [INFO], important steps you need to take care about are highlighted with [IMPORTANT]

## Prerequisites

- You have Java installed, check above which version I have used
...
- Before this application can be started there are some steps to be taken such as setting up a local docker database which are described in the following steps, however the class you will need eventually to run is "CoinApplication.java" but step by step. Here are some general infos which you should notice:
...
- This File is referenced in class `PostgresDatasource.java` in line #14
  ...
  > Flyway comes as an out-of-the-box integration plug in, find more details here: https://flywaydb.org/documentation/plugins/springboot

### [INFO] Create or Update a DB Table within Eclipse
...
> **Note:** The resutl should look something like this:
> `bash-5.0#`
...
### [IMPORTANT] I recommend to administer the db via pgAdmin since it is very easy to use and more user friendly
...
### [IMPORTANT] Ckeck that the src/main/resources/application.yml file has
...
- Access pgadmin again and refresh coin
- Drill down to coin > coin > Schemas > Tables and you should see ~ 25 Tables which have been created
...
- If everything worked then you should be able to refresh in cryptobackup in pgAdmin and see that there is content in the tables.
...
> **Note:** Running the app in a Docker Container requires a change in 'application.yaml' in order to find the database the following line needs to be entered:
